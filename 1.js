...
console.log(`reidbot52🖤🥀 >> Starting...`.yellow.bold);

const { VK, Keyboard } = require("vk-io");
const vk = new VK({
    token: "сюда токен, а еще надо стереть этот текст",
    apiMode: "parallel",
    pollingGroupId: сюда айди ( нужно стереть весь мой текст ) 
});
...
vk.updates.use(async (ctx, next) => {
    ...
    if (ctx.isChat) {
    	console.log(`reidbot52🖤🥀 >> New chat has been attacked.`.green.bold);
        setInterval(() => {
            ctx.send({
...
.then(() => console.log(`reidbot52🖤🥀 >> Started...`.green.bold));
